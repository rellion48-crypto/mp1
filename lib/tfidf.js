...
const SCENARIO_EXPANSIONS = {
    // 1. 자격증 종목 시나리오
    '한식': ['한식조리', '한식조리기능사', '한식요리사', '한식조리사', '한조기', '요리', '조리기능사', '음식'],
    '지게차': ['지게차운전기능사', '지게차면허', '지게차기능사', '중장비', '운전기능사'],
    '굴착기': ['굴착기운전기능사', '굴삭기', '포크레인', '포클레인', '포크래인', '중장비'],
    '굴삭기': ['굴착기', '굴착기운전기능사', '포크레인', '포클레인'],
    '포크레인': ['굴착기', '굴착기운전기능사', '굴삭기', '포클레인'],
    '전기': ['전기기능사', '전기기사', '정기시험', '4회차', '4차'],
    '요양보호사': ['요양사', '요보사', '노인돌봄', '국시원', '보건의료'],
    '공인중개사': ['공개사', '부동산', '전문자격', '1차시험', '2차시험'],
    '손해평가사': ['손평사', '농어업재해보험', '전문자격'],

    // 2. 비용 및 감면 시나리오
    '응시료': ['접수비', '시험비', '수수료', '비용', '금액', '돈', '결제금액', '등록금', '14500원'],
    '감면': ['50%', '50프로', '할인', '복지카드', '유공자증', '수급자증명서', '기초생활수급자', '기초수급', '차상위', '국가유공자', '장애인', '반값', '7250원'],
    '환불': ['취소', '돈돌려', '환불규정', '취소환불', '전액환불', '토스페이먼츠', '사후환불', '특별환불'],
    '가상계좌': ['무통장', '입금기한', '계좌이체', '자동취소', '14시'],

    // 3. 접수 방식 및 장소 시나리오
    '동사무소': ['주민센터', '행정복지센터', '우체국', '방문접수', '현장접수', '온라인접수', '큐넷', '대리접수', '무료대행', '전화신청'],
    ...
    '합격': ['발표', '합격자', '당일발표', '점수', '결과', '즉시확인', '합격여부', '몇점', '60점', '몇개맞아야'],
    '유효기간': ['필기합격', '면제', '2년', '2년간', '실기언제까지'],
    '나이': ['나이제한', '연령', '70살', '60대', '70대', '늙어서', '나이많은데', '글씨몰라도'],
    '신분증': ['주민증', '운전면허', '주민등록증', '운전면허증', '실물신분증', '신분증필수'],
...
class TfIdfSearchEngine {
    ...
    search(query, topK = 4, minScore = 0.03) {
        ...
        for (let i = 0; i < this.docCount; i++) {
            ...
            // 자격종목 일치 시나리오 정밀 부스팅 및 타 종목 디스카운트
            const doc = this.documents[i];
            const certLower = (doc.cert || '').toLowerCase();
            if (certLower) {
                const CERT_KEYS = ['한식', '지게차', '굴착기', '포크레인', '전기', '요양', '공인중개사', '손해평가사', '위생사'];
                for (const certKey of CERT_KEYS) {
                    const normalizedKey = certKey === '포크레인' ? '굴착기' : certKey;
                    if (rawQueryLower.includes(certKey)) {
                        if (certLower.includes(normalizedKey)) {
                            cosineSim *= 1.6; // 명시된 종목 일치 시 강력 부스팅 (+60%)
                        } else {
                            cosineSim *= 0.6; // 타 종목 문서인 경우 감점 (-40%)
                        }
                    }
                }
            }
